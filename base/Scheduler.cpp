...
void Scheduler::schedule() {
    ...
    while (true) {
        ...
        curr.execute();
        processes.pop();
        // Si solo queda el proceso actual y este ha terminado, se agrega uno nuevo antes de dejar la memoría vacía
        if (processes.empty() && !curr.isAlive()) addProcess(Process(maxMemory, maxQuantum));
